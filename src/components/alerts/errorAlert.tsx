...
export const ErrorAlert = () => {
  ...
  if (context.error.state == true) {
    setTimeout(() => {
      context.setError({ state: false, message: "" });
    }, 3500);
  }
      ...
      <div className='flex items-center justify-center px-4 sm:px-0 '>
        <div
          ...
          <div className='items-center sm:flex'>
            ...
            <div className='mr-2 hidden  h-1 w-1 rounded-full xl:block' />
            <p className='text-base '>{context.error.message}</p>
          </div>
          <div className='mt-4 flex justify-end md:mt-0 md:pl-4 lg:pl-0'>
            <span
              onClick={() => context.setError({ state: false, message: "" })}
              className='cursor-pointer text-sm text-black'>
              Dismiss
            </span>
          </div>
